...
> **Note**: It is generally not recommended to generate all the APIs at this time. There are many API endpoints that have issues that will cause the loading of the client to fail. Only after you correct _all_ the issues within the API files will the client load without issues.
...
Writes a static `llms.txt` describing `CanvasSession`, `CanvasClient`, `CanvasAPIError`, pagination behavior, async usage, and common usage patterns.
...
By default, all list endpoints automatically fetch every page of results and return them as a single list. You can override this behavior on any API call using the `per_page` and `page` parameters.

**`per_page`** — sets the page size for that specific request, overriding the `max_per_page` set on the client. When `per_page` is supplied, automatic pagination is disabled and only the requested page is returned.

**`page`** — requests a specific page number. When `page` is supplied, automatic pagination is also disabled.

```python
# Automatic pagination (default) — returns all results as a single list
all_courses = client.courses.list_courses_for_a_user(user_id=123)

# Fetch only the first 10 results
first_page = client.courses.list_courses_for_a_user(user_id=123, per_page=10)

# Fetch a specific page
second_page = client.courses.list_courses_for_a_user(user_id=123, per_page=10, page=2)

# Works the same way with async
first_page = await client.courses_async.list_courses_for_a_user(user_id=123, per_page=10)
```

> **Note:** Supplying either `per_page` or `page` disables automatic pagination for that call. Use the default (no arguments) when you want all results returned automatically.
